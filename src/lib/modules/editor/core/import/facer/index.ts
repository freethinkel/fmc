...
import { classifyText, fontOf, isLive, SEL_SRC, type FieldClass } from "./text";
import { buildRow, numMeta } from "../field";
...
export async function facerToFace(files: File[]): Promise<{ face: Face; skipped: string[] }> {
  ...
  // Facer positions text by its baseline, and its alignment enum (0/1/2) is already the one
  // buildRow takes — everything else about the row is the shared shape.
  async function buildField({ l, cls }: Field, ambient: boolean): Promise<FaceNode> {
    ...
    return buildRow(
      {
        parts: cls.parts,
        font: fontOf(family, Math.round(num(l.size) * s) || 20),
        color: rgba(argb(ambient ? (l.low_power_color ?? l.color) : l.color)),
        x: num(l.x) * s,
        y: num(l.y) * s,
        align: l.alignment ?? 0,
      },
