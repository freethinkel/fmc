...
<header
  class="bg-background sticky top-0 z-50 flex h-(--header-height) w-full shrink-0 items-center border-b"
>
  <div class="flex w-full items-center gap-2 px-4">
    <Sidebar.Trigger class="-ms-1 hidden md:flex" />
    <Separator
      orientation="vertical"
      class="mx-1 hidden data-[orientation=vertical]:h-4 md:block"
    />
    <Breadcrumb.Root>
      <Breadcrumb.List>
        <Breadcrumb.Item>
