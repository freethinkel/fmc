...
import { snapAxis, snapTargets } from "$lib/modules/editor/lib/snap";
import { SNAP_THRESHOLD } from "$lib/modules/editor/shared/constants";
import type { Hit } from "$lib/modules/editor/lib/canvas";
import type { FaceNode } from "$lib/modules/editor/lib/wf";

const hit = (node: FaceNode, x: number, y: number, w: number, h: number): Hit => ({
  node,
  x,
  y,
  w,
...
describe("snapAxis", () => {
  ...
  test("butts up against a neighbour: right edge to that box's left edge", () => {
    const other = snapTargets([hit({ tag: 0x30 }, 200, 0, 60, 40)], {
      tag: 0x33,
    });
...
describe("snapTargets", () => {
  const child: FaceNode = { tag: 0x30 };
  const dragged: FaceNode = { tag: 0x33, subs: [child] };
  const other: FaceNode = { tag: 0x30 };
