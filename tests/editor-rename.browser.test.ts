...
import { parseBin, TAG, unhex } from "$lib/modules/editor/lib/wf";
...
test("renameFace updates the header, the 0x86 node and face.name", async () => {
  ...
  const tailBefore = unhex(editorModel.$editor.getState().face!.nameRaw!)[15];
  ...
  expect(editorModel.$editor.getState().face!.name).toBe(
